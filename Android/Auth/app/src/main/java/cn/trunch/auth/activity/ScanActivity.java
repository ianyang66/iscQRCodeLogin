...
//                DialogUIUtils.showToastTop(result);
                ...
                new UniteApi(ApiUtil.TOKEN_INFO + result, hashMap).post(new ApiListener() {
                    ...
                    public void success(Api api) {
                        ...
                        if (auth.getAuthState() == 0 || auth.getAuthState() == 2) {
                            ...
                            DialogUIUtils.showToastCenter("登錄碼已使用");
                        } else {
                            DialogUIUtils.showToastCenter("登錄碼已過期");
                        }
                        delayStartSpot();
                    }
    ...
    protected void onStart() {
        super.onStart();
        scanView.startCamera();//打開相機
        scanView.showScanRect();//顯示掃描框
        scanView.startSpot();//開始識別二維碼
        //scanView.openFlashlight();//開燈
        //scanView.closeFlashlight();//關燈
    }
